...
@objc(CryptoModule)
class CryptoModule: NSObject, RCTBridgeModule {
  ...
  var keyName: String {
    return "keychain-sample.sampleKeyBio"
//      return useBioSwitch.isOn ? "keychain-sample.sampleKeyBio" : "keychain-sample.sampleKey"
  }
...
//          publicKeyLab
  ...
  private func prepareKey() throws -> Bool {
      defer {
          showPublicKey()
      }
      guard key == nil else {
  ...
  @objc(doEncrypt:resolver:rejecter:)
  func doEncrypt(_ newText: String,
                 _ resolve: RCTPromiseResolveBlock,
                 rejecter reject: RCTPromiseRejectBlock
  ) {
      
      do {
        guard try prepareKey() else {
            reject("1", "2", NSError(domain: "prepareKeyError", code: 500, userInfo: nil))
            return
        }
      } catch let error {
        reject("1", "2.1", NSError(domain: "pepareKeyThrown", code: 500, userInfo: ["error": error.localizedDescription]))
        return
      }
      ...
      var error: Unmanaged<CFError>?
      let clearTextData = newText.data(using: .utf8)!
      cipherTextData = SecKeyCreateEncryptedData(publicKey, algorithm,
                                                 clearTextData as CFData,
                                                 &error) as Data?
      guard cipherTextData != nil else {
      ...
      let cipherTextHex = cipherTextData!.toHexString()
      return resolve("Cipher Text: " + cipherTextHex)
  }
